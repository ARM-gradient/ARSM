...
This file include 3 files folder: rl, toy, and vae.  Code in both the toy and vae folders is written with Python 2.7 and Tensorflow version 1.12.0. Codes in rl folder is written with Python 3.6 and Tensorflow (Eager) version 1.8.0.
...
-- ARSM_Univariate.py contains REINFORCE, AR , ARS, ARSM and Gumbel-Softmax (to Reproduce Figure 1, first run toy_relax_single.py, and then run this file)
...
-- RL_ARSM.py contains ARS and ARSM algorithm on RL task, default task is Cart Pole (can be easily changed to Acrobot and LunarLander by uncommenting corresponding lines)
