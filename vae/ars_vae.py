...
ref_id = np.random.randint(n_class)
pai_slice_ref = tf.slice(pai,begin=[0,0,0,ref_id],size=[-1,-1,-1,1]) 

F_control = 0.
for ss in range(n_class):
    pai_s = swap(pai,ref_id,ss)
    F_s = Fn(pai_s,prior_logit0,z_concate,x_star_u)
    F_control = F_control + F_s
F_control = F_control/n_class


for mm in range(1,n_class):
    pai_m = swap(pai,ref_id,mm)
    F_m = Fn(pai_m,prior_logit0,z_concate,x_star_u)
    grad_m = tf.expand_dims(tf.expand_dims(F_m-F_control,2),3)*(1-n_class*pai_slice_ref)
    if mm == 1:
        alpha_grads = grad_m
    else:
